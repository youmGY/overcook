...
반전 횟수만으로는 부족하다. 작은 떨림도 반전으로 잡힐 수 있기 때문에, **최근 데이터의 진폭**이 임계값 이상인지도 확인한다.

```python
r_y_amp = _recent_amplitude(st.wy, amp_window)  # deque 내 max - min
...
손이 **30프레임(FPS 보정됨) 동안 움직이지 않으면** 모든 모션 상태를 초기화한다.

```python
_STILL_SPEED_MAX = 0.002  # 이 속도 이하면 "정지"로 판단
_STILL_RESET_FRAMES = 30  # 30프레임 연속 정지 시 리셋
```
