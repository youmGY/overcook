...
finger_3_another 클래스 추가 버전.

finger_3        : 엄지 + 검지 + 중지 펴기 (HaGRID three2)
finger_3_another: 검지 + 중지 + 약지 펴기 (HaGRID three)

사용법:
    python train/train_60f_with_3another.py
...
LABEL_REMAP = {
    ...
    16: 2,   # three2 → finger_3 (엄지+검지+중지)
    15: 3,   # three → finger_3_another (검지+중지+약지)
    3:  4,   # four → finger_4
    9:  5,   # palm → finger_5
    13: 5,   # stop → finger_5
    14: 5,   # stop_inverted → finger_5
    4:  6,   # like → thumbs_up
    1:  6,   # dislike → thumbs_up
    2:  7,   # fist → fist
}
...
# HaGRID three (index 15) 샘플 수 확인
three_count = (labels_raw == 15).sum()
print(f"\nHaGRID 'three' (finger_3_another) 원본 샘플: {three_count}개")
...
tmp_path = os.path.join(onnx_dir, 'gesture_mlp_60f_8cls_tmp.onnx')
onnx_path = os.path.join(onnx_dir, 'gesture_mlp_60f_8cls.onnx')
