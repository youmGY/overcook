"""
웹캠 제스처 인식 테스트 (60차원: 정규화 좌표 56 + 엄지 각도 코사인 4)

사용법:
    python train/test_webcam_60f.py
    (train/gesture_mlp_60f.onnx 필요)
    q / ESC 종료
"""
...
# ── 정규화 & 피처 (train_60f_norm_thumb.py 와 동일) ──────────
...
def landmarks_to_features(lm_norm: np.ndarray) -> np.ndarray:
    ...
    # 엄지 각도 코사인 4개
    feats.append(_cos_angle(lm_norm[1], lm_norm[0], lm_norm[5]))  # 벌림
...
def main():
    ...
    if not os.path.exists(onnx_path):
        print(f"[ERROR] {onnx_path} 없음. train_60f_norm_thumb.py 먼저 실행하세요.")
        sys.exit(1)
    ...
    while True:
        ...
        cv2.imshow("Gesture Test (60-dim norm+thumb)", frame)
